...
|`wordcloud`|`install.packages("tidytext")`|
...
2. Print the orginal and the new data frame and compare their dimensions. How much has the object grown?
...
4. Using `wordcloud()`, plot a word cloud of the words of game of thrones with words that occue more than 50 times (use `filter()`).
...
5. You probably have noticed that the word cloud was dominated by some very unreavealing words. To obtain a representation of what the show is about remove socalled `stop_words` from `got_counts` using the `anti_join` function. See below.
