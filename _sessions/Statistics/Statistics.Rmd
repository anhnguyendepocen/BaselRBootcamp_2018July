...
To conduct a regression analysis, use `glm()` or `lm()`. `lm()` is for simple linear regression, while `glm()` is generalized.
...
Explore statistical objects using many *generic* functions such as `print()`, `summary()`, `predict()` and `plot()`.
...
Many (but not all) statistical objects can be used with `predict()`

`predict(model, newdata)` allows you to use the `model` to predict outcomes from `newdata`.
