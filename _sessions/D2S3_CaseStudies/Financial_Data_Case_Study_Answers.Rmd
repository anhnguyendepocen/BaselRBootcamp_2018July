...
5. There's something wrong. First, we want the columns to be called `Date` and `Rate` (and not be taken from the first row). To fix this this, load the data again with the following additional arguments:
...
6. Ok your datasets should be correct now! Get a first impression of the them using `print()`, `typeof()` and `str()`. What type are the data objects and what variables do they contain?
