...
1. Open a new R script and save it as a new file in your `R` folder called `dimarta_casestudy.R`. At the top of the script, using comments, write your name and the date. Then, load the `tidyverse` package. Here's how the top of your script should look:
...
8. Using `write_csv()`, save `dimarta_df` in a new `.csv` file in your `data` folder called `dimarta.csv`
...
9. Using the `mean()` function, calculate the mean age of all patients.

```{r}
mean(dimarta_df$age)
```

10. Using the following template, find out how many male and female patients were in the trial.
...
11. Now, using similar code, find out how many patients were assigned to each study arm.
...
12. Find out how many men and women were assigned to each study arm (Hint: You can use very similar code to what you used above, just add a second grouping variable!)
...
13. Using `mutate()`, add a new column to the `dimarta_df` data called `histamine_change` that shows the *change* in patient's histamine levels from the start to the end of the trial (Hint: just subtract `histamine_start` from `histamine_end`!)
...
14. Using `mutate()` again, add a new column to `dimarta_df` called `qol_change` that shows the change in patient's quality of life.
...
15. Calculate the percentage of patients who tested positive for each of the three biomarkers using the following template (Hint: If you calculate the `mean()` of a logical vector, you will get the percentage of TRUE values!)
...
16. Were there different distributions of age in the different trial sites? To answer this, separately calculate the mean and standard deviations of patient ages in each site. (Hint: group the data by `site`, then calculate two separate summary statistics: `age_mean = mean(age)`, and `age_sd = sd(age)`.
...
17. Calculate the mean change in histamine results separately for each study site 
...
18. Calculate the mean change in histamine results (`histamine_change`) for each study arm. Which study arm had a largest decrease in histamine?
...
19. Calculate the mean change in quality of life (`qol_change`) for each study arm. Do the results match what you found with the histamine results?
