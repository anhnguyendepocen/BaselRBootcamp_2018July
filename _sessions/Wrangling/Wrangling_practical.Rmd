...
7. Use the the `summary()` or `skim()` function (from the `skimr` package) to get more details on the datasets.
...
1. Printthe names of the `trial_act` data with `names()`
...
1. Using the `select()` function, select only the column `age` and print the result (but don't assign it to anything). Do you see only the age column now?
