...
|File | Rows | Columns |
|:----|:-----|:------|
|[trial_act.csv](https://raw.githubusercontent.com/therbootcamp/BaselRBootcamp_2018July/master/_sessions/_data/complete/trial_act.csv) | 2139 | 27 |
|[trial_act_demo_fake](https://raw.githubusercontent.com/therbootcamp/BaselRBootcamp_2018July/master/_sessions/_data/complete/trial_act_demo_fake.csv)| 2139 | 3|
...
1. Open your R project. It should already have the folders `1_Data` and `2_Code`. Make sure that the data files listed in the `Datasets` section above are in your `1_Data` folder
...
7. Use the `skim()` function (from the `skimr` package) to get more details on the datasets.
...
1. Look at the names of the `trial_act` data with `names()`

2. Using `rename()`, change the column name `wtkg` in the `trial_act` dataframe to `weight_kg` (to specify that weight is in kilograms). Be sure to assign the result back to `trial_act` to change it!
...
1. Using the `select()` function, select only the column `age` and print the result. Do you see only the age column now?
...
3. If you look at the `gender` column, you will see that it is numeric. Change the column so it shows gender as a string, where0 = "female" and 1 = "male". To do this, use a combination of `mutate()` and `case_when`:
...
4. The column `arms` is also numeric and not very meaningful. Change `arms` so that it is a character column indicating the actual names of the trial arms. Here is a table of the mapping
...
4. Create a new dataframe called `trial_gender` groups the data based on `gender` and calculates the same summary statistics as you did for `trial_arm`

5.  Create a new dataframe called `trial_arm_gender` that calculates the same summary statistics for all the groups `gender` and `arms`. Hint: Just add a second grouping variable!
...
2. Now, in the `trial_act` dataset, group the data by `arm` and calculate the mean of all numeric columns using `summerise_if()`. Here's another scoped function in action `mutate_if()` in action:
