...
Task: Calculate the mean of a vector of `scores` and round to 0 digits.
...
How to change column names with `rename()`.
...
Add new columns (or change existing ones) with `mutate()`.
...
Use `case_when()` with `mutate()` to <high>define new columns based on logical conditions</high>
...
<high>Select everything except</high> certain columns with `-`.
...
Drop age_months and age_decades colummns.
