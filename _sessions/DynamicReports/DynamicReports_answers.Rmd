...
Here are the dataset(s) I will explore in this document:

|Name|Description|Rows|Columns|
|:------|:---------------------------------|:--------|:------|
|`mcdonalds.csv`| Menu information from [McDonalds](https://www.mcdonalds.ch/de/)|`r nrow(mcdonalds)`|`r ncol(mcdonalds)`|
|`happiness.csv`| Country happiness statistics from the [World Health Organisation](http://www.who.int/whr/previous/en/)|`r nrow(happiness)`|`r ncol(happiness)`|


# McDonalds

```{r, out.width = "30%", fig.cap="Source: Wikipedia"}
knitr::include_graphics("https://upload.wikimedia.org/wikipedia/commons/thumb/3/36/McDonald%27s_Golden_Arches.svg/2000px-McDonald%27s_Golden_Arches.svg.png")
```

The `mcdonalds` data contains `r ncol(mcdonalds)` pieces of information for `r nrow(mcdonalds)` menu items. The data originally come from [Kaggle](https://www.kaggle.com/mcdonalds/nutrition-facts).
...
Here are are few key descriptive statistics from the data:

- The mean number of calories in a McDonalds menu item is `r mean(mcdonalds$Calories)`
- The maximum number of calories in a McDonalds menu item is `r max(mcdonalds$Calories)` for  `r mcdonalds$Item[mcdonalds$Calories == max(mcdonalds$Calories)]`

## Calories

The following plot is a histogram showing the distribution of calories across all menu items
...
```{r, out.width = "30%", fig.cap="Source: Miratelinc.com"}
knitr::include_graphics("http://www.miratelinc.com/blog/wp-content/uploads/2015/05/10-happiest-countries-2015.jpg")
```


The `happiness` data contains `r ncol(happiness)` pieces of information for `r nrow(happiness)` countries. The data originally come from the World Happiness Report but were taken from [Kaggle](https://www.kaggle.com/unsdsn/world-happiness). Here is a table showing a few columns from the data
...
- The mean happiness score was `r mean(happiness$Happiness.Score)`


## Happiness

The following plot is a histogram showing the distribution of Happiness scores across all menu items
