...
- Explore the `XXX_train` dataframe with a combination of `skim()`, `names()`, `summary()` and other similar functions.
...
- Train one or more models on the training data. Start with one model, then gradually try more. For each model, assign the result to a new training object called `XX_train` (e.g.; `rf_train` for random forests, `glm_train` for standard regression.)
...
A1. Open your R project. It should already have the folders `1_Data` and `2_Code`. Make sure that the all of the datasets you need for this practical are in your `1_Data` folder

```{r}
# Done!
```

A2. Open a new R script and save it as a new file called `ml_practical.R` in the `2_Code` folder. At the top of the script, using comments, write your name and the date. Then, load the all of the packages you'll need for this practical. Here's how the top of your script should look:
...
B1. Follow Steps A through I above, to create your best decision tree model predicting price. Call the model `rpart_price`

B2. Do the same with random forests using `method = "rf"`. Call the model `rf_price`

B3. Do the same with regularised regression using `method = "glmnet"`. Call the model `glmnet_price`

B4. Which of the three models was the best a predicting price? Were they very different? Is one easier to interpret than the other?

B5. Go to [http://topepo.github.io/caret/available-models.html](http://topepo.github.io/caret/available-models.html) and find the craziest looking model you can find that works for regression tasks. Then, try it on the price dataset and see how well it works! Does it do much better than the models you already tried?

B6. Repeat your analyses, but now train your models using 10 fold cross validation by specifying `trainControl(method = "cv", number = 10, repeats = 1)`. How do the accuracy of the models change? Do they become more accurate when you use 10 fold cross validation compared to when you didn't?
...
C1. Follow Steps A through I above, to create your best decision tree model predicting diagnosis. Call the model `rpart_diagnosis`

C2. Do the same with random forests using `method = "rf"`. Call the model `rf_diagnosis`

C3. Do the same with nearest neighbors using `method = "knn"`. Call the model `knn_diagnosis`

C4. Which of the three models was the best a predicting diagnosis? Were they very different? Is one easier to interpret than the other?

C5. Go to [http://topepo.github.io/caret/available-models.html](http://topepo.github.io/caret/available-models.html) and find the craziest looking model you can find that works for classification tasks. Then, try it on the heartdisease dataset and see how well it works! Does it do much better than the models you already tried?

C6. Repeat your analyses, but now train your models using 10 fold cross validation by specifying `trainControl(method = "cv", number = 10, repeats = 1)`. How do the accuracy of the models change? Do they become more accurate when you use 10 fold cross validation compared to when you didn't?
