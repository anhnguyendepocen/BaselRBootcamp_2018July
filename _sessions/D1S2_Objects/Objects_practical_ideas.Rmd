...
1. Using the `data.frame()` function, create a dataframe called `ten_baselers` that contains each of vectors you just created: `id`, `sex`, `height`, `weight`, `tall_180`

```{r, eval = FALSE, echo = TRUE}
XX <- data.frame(XX, XX, XX, XX, XX) 
```

2. Print your `ten_baselers` object to see how it looks! Does it contain all of the vectors?

3. Using the `dim()` function, print the number of rows and columns in your dataframe. Do you get 10 rows and 5 columns?

4. What is the class of your `ten_baselers` object? Use the `class()` function to find out!

5. Use the `summary()` function to print descriptive statistics from each column of `ten_baselers`

6. Using the `$` operator, print the `age` column from the `ten_baselers` dataframe.

7. Calculate the maximum `age` value from the `ten_baselers` dataframe. Do you get the same result from when you calculated it from the original vector `age`?
