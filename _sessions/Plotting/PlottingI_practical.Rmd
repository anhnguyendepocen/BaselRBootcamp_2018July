...
E1. Create a scatterplot showing the relationship between Cholesterol and Protein 

E2. Color the points according to their Calories by specifying the `col` aesthetic

E3. Change the colors by including the additional argument ` + scale_colour_gradient(low = "blue", high = "red")`

E4. Use a gray color palette by using `scale_color_grey()` instead of `scale_colour_gradient()`
...
F1. Start with the following template
...
F2. Try adding labels to the plot indicating which item each point represents by adding `+ geom_text()`.

F3. Where are the labels? Ah we didn't tell ggplot which column in the data represents the item descriptions. Fix this by specifying the `label` aesthetic in your first call to the `aes()` function. That is, include `label = Item` underneath `col = XX`. Now you should see lots of labels!

F4. Customize your `geom_text()` by including the arguments: `geom_text(col = "black", check_overlap = TRUE, hjust = "left")`

F5. Using the `data` argument in `geom_text()` specify that the labels should only apply to items over 1100 calories (hint: `geom_point(data = mcdonalds %>% filter(XX > XX))`)
...
G1. Create the following plot showing the mean number of calories for each menu category using the following template
...
H1. It's time to save your favorite plot to an image file! Pick your favorite plot you've created so far. Then, assign the plot to a new object called `mcdonalds_gg` using `mcdonalds_gg <- ggplot(...)`

```{r, eval = FALSE, echo = TRUE}
mcdonalds_gg <- ggplot(...) # Include your plotting code here
```

H2. Evaluate your `mcdonalds_gg` object to see that it does indeed contain your plot.

H3. Save your plot to a pdf file called `mcdonalds.pdf` using `ggsave()`. When you finish, open your plot to see how it looks! 
...
H4. Play around with the `width` and `height` arguments to change the dimensions of the plot.

H5. Customize your code to create a jpeg image called `mcdonalds.jpeg`
